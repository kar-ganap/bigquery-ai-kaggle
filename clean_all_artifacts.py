...
def clean_pipeline_artifacts():
    ...
    PRESERVE_TABLES = {
        ...
        'text_embedding_model',  # Infrastructure - will be recreated if needed
    }
...
def clean_local_artifacts():
    """Clean up local output files (preserving checkpoints)"""
    import os
    import glob

    print(f"\n🧹 Cleaning Local Artifacts (preserving checkpoints)...")

    # Clean temporary files but preserve checkpoints
    patterns_to_clean = [
        "data/output/*.log",              # Log files
        "data/output/*.json",             # Temporary JSON files (not in clean_checkpoints)
        "data/output/sql_dashboards_*",   # Generated SQL dashboard directories
    ]
    ...
    if preserved_count > 0:
        print(f"   💾 Preserved {preserved_count} checkpoint files (high watermark solutions)")
...
def main():
    ...
    cleanup_success = clean_pipeline_artifacts()
